...
BlockList::BlockList(std::string other_file,std::string main_file) : other_file(other_file){
    file.open(other_file,std::ifstream::in | std::ifstream::binary);
    ...
    file.close();
    file.clear();
    file.open(main_file,std::fstream::app | std::fstream::binary);
    file.close();
    file.clear();
    file.open(main_file,std::fstream::in | std::fstream::out | std::fstream::binary);
}

BlockList::~BlockList() {
    file.close();
    file.clear();
    file.open(other_file,std::ofstream::out | std::ofstream::binary);
...
void BlockList::insert(const char *index,const int &value) {
...
void BlockList::remove(const char *index,const int &value) {
...
std::vector<int> BlockList::find(const char *index) {
    std::vector<int> ans;
